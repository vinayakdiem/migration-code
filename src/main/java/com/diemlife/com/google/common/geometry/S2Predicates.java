...
public final strictfp class S2Predicates {
  ...
  private static final double DBL_ERR = S2.DBL_EPSILON / 2;
   ...
   */
  // TODO(eengle): Triage with Real when double math isn't precise enough, before using BigDecimal.
  private static final double T_ERR = DBL_ERR;

  /** A predefined S1ChordAngle representing (approximately) 45 degrees. */
  private static final S1ChordAngle DEG_45 = S1ChordAngle.fromLength2(2 - S2.M_SQRT2);
  ...
  public static class Sign {
    ...
    public static int stable(S2Point a, S2Point b, S2Point c) {
      ...
      final double detErrorMultiplier = 3.2321 * S2.DBL_EPSILON; // see above
      double det;
      double maxError;
      if (ab2 >= bc2 && ab2 >= ca2) {
    ...
    public static int exact(S2Point a, S2Point b, S2Point c, boolean perturb) {
      ...
      // but are faster than BigDecimal techniques.
      ...
      // Check the determinant using BigDecimal, which is exact but very slow.
      // Save the cross product for use in symbolic perturbation if necessary.
      BigPoint xa = big(a);
      BigPoint xb = big(b);
      BigPoint xc = big(c);
     ...
     *   <li>The points must be distinct, with A < B < C in lexicographic order.
     * </ul>
     *
   ...
   * the point O. You can think of this as testing whether A <= B <= C with respect to a continuous
   * CCW ordering around O.
   *
   * <p>Properties:
   *
   * <ol>
   *   <li>If orderedCCW(a,b,c,o) && orderedCCW(b,a,c,o), then a == b
   *   <li>If orderedCCW(a,b,c,o) && orderedCCW(a,c,b,o), then b == c
   *   <li>If orderedCCW(a,b,c,o) && orderedCCW(c,b,a,o), then a == b == c
   *   <li>If a == b or b == c, then orderedCCW(a,b,c,o) is true
   *   <li>Otherwise if a == c, then orderedCCW(a,b,c,o) is false
   * </ol>
   */
  ...
  /**
   * Returns -1, 0, or +1 according to whether AX < BX, A == B, or AX > BX respectively. Distances
   * are measured with respect to the positions of X, A, and B as though they were reprojected to
   * lie exactly on the surface of the unit sphere. Furthermore, this method uses symbolic
   * perturbations to ensure that the result is non-zero whenever A != B, even when AX == BX
   * exactly, or even when A and B project to the same point on the sphere. Such results are
   * guaranteed to be self-consistent, i.e. if AB < BC and BC < AC, then AB < AC.
   */
   ...
   * sphere.
   */
  // TODO(eengle): Make this public once the library has moved onto it.
   ...
   *
   * <p>Requires that A and B do not project to antipodal points (e.g., A != -B). This can occur if
   * for example A == S * B, for some constant S < 0.
   *
  ...
  public static int compareEdgeDistance(S2Point x, S2Point a, S2Point b, double r2) {
    ...
    // This catches the most common case. The full test is in EXACT.
  ...
  static class CompareEdgeDistance {
     ...
     */
    // TODO(eengle): x.angle(a)+a.angle(b) < r is usually false, so test that first?
    public static int triage(S2Point x, S2Point a, S2Point b, double r2) {
      ...
      double n1Error = ((3.5 + 8 / Math.sqrt(3)) * n1 + 32 * Math.sqrt(3) * DBL_ERR) * T_ERR;
    ...
    static int triageLineSin2(
      ...
      // limit is small (e.g., S2.kIntersectionError).
      ...
      double xDn2 = xDn * xDn;
      double c1 =
          ((3.5 + 2 * Math.sqrt(3)) * n1 + 32 * Math.sqrt(3) * DBL_ERR) * T_ERR * Math.sqrt(ax2[0]);
      double xDn2Error = 4 * T_ERR * xDn2 + (2 * Math.abs(xDn) + c1) * c1;
    ...
    static int triageLineCos2(S2Point x, double r2, S2Point n, double n1, double n2) {
      ...
      double m1Error = ((1 + 8 / Math.sqrt(3)) * n1 + 32 * Math.sqrt(3) * DBL_ERR) * DBL_ERR;
    ...
    static int exactLineEndpoints(S2Point x, S2Point a, S2Point b, double r2) {
      return Math.min(compareDistance(x, a, r2), compareDistance(x, b, r2));
    }
  ...
  static class CompareEdgeDirections {
    ...
    public static int triage(S2Point a, S2Point b, S2Point c, S2Point d) {
      ...
      double cosError =
          ((5 + 4 * Math.sqrt(3)) * len1 * len2 + 32 * Math.sqrt(3) * DBL_ERR * (len1 + len2))
              * T_ERR;
  ...
  static class EdgeCircumcenterSign {
    ...
    public static int triage(S2Point p, S2Point q, S2Point a, S2Point b, S2Point c, int abc) {
      ...
      double nxError = ((1 + 2 * Math.sqrt(3)) * nxLen + 32 * Math.sqrt(3) * DBL_ERR) * T_ERR;
    ...
    public static int exact(BigPoint p, BigPoint q, BigPoint a, BigPoint b, BigPoint c, int abc) {
      ...
      int lhsSign2 = Math.max(-1, Math.min(1, lhsSign3 - rhs3Sign));
      ...
      int result = Math.max(-1, Math.min(1, lhsSign2 - rhsSign2));
      if (result == 0 && lhsSign2 != 0) {
        ...
        result = Math.max(-1, Math.min(1, lhsSign4 - rhs4.signum()));
  ...
  static class VoronoiSiteExclusion {
    ...
    public static Excluded triage(S2Point a, S2Point b, S2Point p, S2Point q, double r2) {
      ...
      double dnError = ((3.5 + 2 * Math.sqrt(3)) * n1 + 32 * Math.sqrt(3) * DBL_ERR) * T_ERR;
      ...
      double ra2Error =
          (8 * DBL_ERR + 4 * T_ERR) * aDn2
              + (2 * Math.abs(aDn) + aDnError) * aDnError
              + 6 * T_ERR * n2sin2r;
      ...
      double rb2Error =
          (8 * DBL_ERR + 4 * T_ERR) * bDn2
              + (2 * Math.abs(bDn) + bDnError) * bDnError
              + 6 * T_ERR * n2sin2r;
      ...
      double sinDError =
          (4 * DBL_ERR + (2.5 + 2 * Math.sqrt(3)) * T_ERR) * aXb1 * n1
              + 16 * Math.sqrt(3) * DBL_ERR * T_ERR * (aXb1 + n1);
      ...
      double cosDError =
          ((8 * DBL_ERR + 5 * T_ERR) * Math.abs(aDn) + aDnError) * Math.abs(bDn)
              + (Math.abs(aDn) + aDnError) * bDnError
   ...
   */
  // TODO(eengle): Update callers to instead use S2.robustCrossProd(b,a).neg()?
  ...
  private static double sin2DistanceError(double x) {
    return (21 + 4 * Math.sqrt(3)) * DBL_ERR * x
        + 32 * Math.sqrt(3) * DBL_ERR * DBL_ERR * Math.sqrt(x)
  ...
  private static S2Point circumcenter(S2Point a, S2Point b, S2Point c, double[] error) {
    ...
    error[0] =
        ((16 + 24 * Math.sqrt(3)) * T_ERR + 8 * DBL_ERR * (abLen + bcLen)) * nabLen * nbcLen
            + 128 * Math.sqrt(3) * DBL_ERR * T_ERR * (nabLen + nbcLen)
